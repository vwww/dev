  ...
  {#each results as pastGame}
    <li class="list-group-item">
      <div>
        ...
        {pastGame.moveSum.toLocaleString()} &equiv; <span class="badge bg-success">{pastGame.winner}</span> (mod {pastGame.teams.length})
      </div>
      {#each pastGame.teams as team, id}
        <div>
          <span class="badge me-1 bg-{team.winner ? 'success' : 'danger'}">#{id}</span> <span class="badge bg-primary">{team.moveSum}</span>:
          {#each team.players as player}
            <span class="badge me-1 bg-secondary">{player.name} ({player.cn})</span>
            <span class="badge me-2 bg-primary">{player.move}</span>
          {/each}
        </div>
      {/each}
    </li>
