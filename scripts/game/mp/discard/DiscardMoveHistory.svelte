...
{#each moves as m}
  <br>
  {#if m.type === 'move'}
    <span class="badge bg-{playerColor(m.playerIsMe)}">{m.playerName}</span> uses <span class="badge bg-light">{getCardName(m.move, ll)}</span>
    {#if m.move !== 4 && m.move < 7}
      on
      {#if m.targetIsPlayer}
        <span class="badge bg-dark">self</span>
      {:else}
        <span class="badge bg-{playerColor(m.targetIsMe)}">{m.targetName}</span>
      {/if}
    {/if}
    {#if m.move === 1 && m.targetValid}
      who has {m.info < 0 ? '' : 'no'} <span class="badge bg-{m.info < 0 ? 'danger' : 'warning'}">{getCardName(Math.abs(m.info), ll)}</span>{m.info < 0 ? '!' : '.'}
    {:else if m.move === 2 && m.targetValid}
      to see the hand.
    {:else if m.move === 3 && m.targetValid}
      {#if m.info}
        and {m.info < 0 ? 'loses with' : 'beats'} <span class="badge bg-light">{getCardName(Math.abs(m.info), ll)}</span>.
      {:else}
        and matches hands!
      {/if}
    {:else if m.move === 4}
      to become <span class="badge bg-info">IMMUNE</span>!
    {:else if m.move === 5}
      who destroys <span class="badge bg-{m.info === 8 ? 'danger' : 'warning'}">{getCardName(m.info, ll)}</span>{m.info === 8 ? '!' : ' and draws.'}
