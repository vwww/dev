  ...
  connect (): void {
    ...
    PlayerIO.authenticate(GAME_ID, CONNECTION_ID, { userId: 'guest' }, {},
      (client) => {
        ...
        const joinMatchMakingRoom = (): void => {
        ...
        const playGameRoom = (roomId: string): void => {
          client.multiplayer.createJoinRoom(roomId, 'slime', true, null, null, (connection) => {
            ...
            connection.addMessageCallback('nextRound', (message) =>
              this.game.processServerRoundStart(message.getBoolean(0)))
            ...
            // send init
            const name = this.game.getUserName()
            const color = this.game.getUserColor()
