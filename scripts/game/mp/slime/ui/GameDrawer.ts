...
export class GameDrawer {
  ...
  constructor (readonly game: Game, private readonly getNetStatus: () => ReadyState) { }
  ...
  drawSlimer (ctx: CanvasRenderingContext2D, W: number, H: number, x: number, y: number,
    color: string | CanvasGradient | CanvasPattern, name: string, p2: boolean, trace?: boolean): void {
    const b = this.game.getBall()
    ...
    ctx.fill()
    // draw pupil
    const dx = (this.flipP1 ? 2 - (b.x + b.xe) : b.x + b.xe) - ex
