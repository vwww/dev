...
import { winnerMapToNum, winnerMapInvert, Winner, WinnerMap, numToWinnerMap } from '@gc/t3/game'
...
function memoTitle (memo: MemoEntry, gameType: number): string {
  const countPathsRaw = memo[1]
  const countPaths = remapPathCount(countPathsRaw, numToWinnerMap(gameType))
...
function winInfo (memo: MemoEntry, result: number, gameType2: number): Entry {
  ...
  const entry = memoEntry[gameType2]
...
$: gameTypeTie = winnerMapToNum(winnerMap.map(w => ((i ^ (w === 3 ? 0 : 1)) & 1) + 1) as WinnerMap)
$: info = !memo ? emptyInfo
  : [
...
<tr>
  <td title={memo && memoTitle(memo, gameType)}>#{i + 1}</td>
